...
```
上传 1-9 张图（可选逐图裁剪；未裁剪图以"统一形状"为默认（自由矩形），打印尺寸整批统一）→ 提交 → 自动修图 → 单张下载或 ZIP 批量打包（桌面下载/复制）→ 发回聊天窗口
```

结果回显走服务端 512px 预览（`?preview=1` 缓存），悬停放大/下载/复制取全幅成品（第四十次修订；移动端长按缩略图存到的是 512px 预览，成品请用「下载」按钮）；处理中可「取消任务」放弃等待（会话级，后台批次由 TTL 清理，第三十七次修订）。
...
| 尺寸缩放 | 按打印尺寸档（4/6/8/10/12 寸 + 自定义 5-33cm，上限=打印机 A3+ 幅面）缩放到 @300DPI 目标短边 | 缩小本地 INTER_AREA；放大走佐糖 scale-pro 高级变清晰（sync=0 异步，服务端定倍）；失败记 failed 前端红显**不交付**（插值废图不是交付，第十七次修订） |
...
| 描边 | **外边缘描边**（第三十二次修订）：形状外扩线宽的灰环画在画布外扩边上——内容零接触、环外全透明，沿线剪=线随废料丢弃 | **按段判定部分环**（第四十一次修订）：打印不印白墨，边界白段看不见补灰线、彩段看得见不画——纯白底=整环、混合底=部分环、整圈彩色边界=不画；线宽批级可选 1–2mm（第三十四次修订，默认跟随配置）；矩形四角=线宽级圆角（等宽环） |

管线全程**原始域**处理（同图不同形状共享全部缓存与模型外呼）；棋盘格图自动换序
先填充再去水印（避免去水印重绘格子致填充判定失效）。
步骤记档语义（2026-08-27 定案）："该做但没做成"（欠费/超时）记 failed 前端红显
"执行失败"；"不需要做"记 skipped——去水印/填充 failed 仍原图零误伤交付，**放大
failed 整图不交付**（第十七次修订，插值废图不是交付）；放大倍率过高时黄标提示
"建议提供更大尺寸的原图"（suggest-larger-source，不阻塞交付；heavy-watermark
提示线已撤销）。
...
- **前端**：无构建链原生 HTML/CSS/JS 单页（批级形状/尺寸/描边线宽三个自绘下拉 + 逐图 cropper.js 裁剪弹层），同进程静态挂载
...
├── core/         # 基础设施：config(pydantic-settings) / database(SQLite 连接) / executor(线程池单例) /
│                 # http(共享事件循环 HTTP 客户端) / api_throttle(第三方并发闸) / logger(双通道日志)
...
tests/            # pytest（API 校验 / 管线阶段 / 描边像素 / 填充生成 / TTL 恢复 / HTTP 线程模型 / 并发闸）
deploy/           # 生产部署（Docker Compose + Nginx 网关 + 部署文档）
...
```bash
# 1. 启动（自动建 .venv、装依赖、复制 .env 模板；会先清掉 data/ 下批次/缓存/库——保留 data/logs/——并杀端口占用进程）
./start.sh
...
> `start.sh` 的 `run` 分支每次启动会清理 `data/` 下批次/缓存/库文件，**保留 `data/logs/`**
> （崩溃重启排障要日志；第八次修订口径）。生产部署保留数据（TTL 自动清理）时，
> 注释掉脚本里的 `clear_runtime_data` 调用。
...
| `PT_WM_PRECHECK_ENABLED` / `PT_WM_API_ENABLED` / `PT_SCALE_ENABLED` / `PT_FILL_GEN_ENABLED` | 各外部步骤总开关（预检/石榴去水印/佐糖超分/生成填充）——**配 key 外还须置 true 才外呼** | false |
| `PT_WM_PRECHECK_KEY` / `PT_SHILIU_API_KEY` | 百炼 VL 预检 / 石榴去水印 Key | 空（预检未配=整步跳过；石榴未配但 VL 判有水印=failed 红显原图交付） |
