...
| 裁剪 | 按前端声明（形状+框）运行时执行：框裁外接区 + 形状掩膜塑形（形状外 alpha=0） | 统一形状**后改优先**（第三十次修订）：切换即全局重声明（逐图已设的形状/框一并重置为整图塑形）；之后逐图再裁剪则优先到下次统一切换；后端只收声明不收像素；同图切形状零重复外呼 |
| 描边 | 白底判定 + 线带净空判定（内容越线不画——裁切线绝不可压图案）→ rembg 图案分割 → 形状边界内缩灰线（1.5mm@300DPI 打印裁切参考线） | 非白底/线带非净空 skipped；rembg 不可用退灰度阈值法 |
