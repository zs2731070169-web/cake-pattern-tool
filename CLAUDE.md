...
5. **描边**（第三十二次修订：**外边缘描边**）：画布外扩一个线宽承接外环（环带=形状外扩线宽），覆盖写灰；**形状内容区一个像素不动**（旧内缩画法线压内容已废，git 历史可溯）；环外全部 alpha=0，沿线剪=线随废料丢弃。白底判定/线带净空判定/形状外渗出清洗全线退役（线永不接触内容，无条件执行）。crop 关闭时回退 rectangle 整幅外环（直通契约）；矩形四角=线宽级圆角（椭圆核等宽环的正确形态）。
