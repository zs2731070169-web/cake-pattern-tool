...
5. **描边**（第三十二次修订：**外边缘描边**）：画布外扩一个线宽承接外环（环带=形状外扩线宽），覆盖写灰；**形状内容区一个像素不动**（旧内缩画法线压内容已废，git 历史可溯）；环外全部 alpha=0，沿线剪=线随废料丢弃。**按段白度判定·部分环**（第四十一次修订，替代第三十三次整图"非黑即白"门——白底但图案贴边被冤杀）：打印不印白墨，形状边界等分 N 段逐段测边带背景亮度中位数，白段（≥235）补线、彩段看得见不画——纯白底=整环、混合底=部分环（都记 done）、整圈彩=skipped；段内 rembg 分割遗漏只污染所在段不拖垮全图。线带净空判定/形状外渗出清洗维持退役。crop 关闭时回退 rectangle 整幅外环（直通契约）；矩形四角=线宽级圆角（椭圆核等宽环的正确形态）。**线宽批级可选**（第三十四次修订：前端"统一描边"下拉 1–2mm 步进 0.1 写入 `crop_meta.outline_width_mm`，管线解析透传，非法/缺失回退 `PT_OUTLINE_WIDTH_MM`）。
