...
5. **描边**（第三十二次修订：**外边缘描边**）：画布外扩一个线宽承接外环（环带=形状外扩线宽），覆盖写灰；**形状内容区一个像素不动**（旧内缩画法线压内容已废，git 历史可溯）；环外全部 alpha=0，沿线剪=线随废料丢弃。**白底判定保留**（第三十三次修订：非白底边界本来就看得见，不加线——门管"画不画"、外环管"怎么画"，正交）；线带净空判定/形状外渗出清洗维持退役。crop 关闭时回退 rectangle 整幅外环（直通契约）；矩形四角=线宽级圆角（椭圆核等宽环的正确形态）。**线宽批级可选**（第三十四次修订：前端"统一线宽"下拉 1–2mm 步进 0.1 写入 `crop_meta.outline_width_mm`，管线解析透传，非法/缺失回退 `PT_OUTLINE_WIDTH_MM`）。
